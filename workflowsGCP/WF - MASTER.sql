    ...
    steps:
        - initVariables:
            assign:
                ...
                - var_name_prc: prc_load_m_contracargo_mastercard

        - createMap:
            assign:
                - procedures:
                    m_contracargo_mastercard:
                        name: ${var_project_operation + "." + var_dataset_master_stage_risk + "." + var_name_prc}

        - 01_prc_load_table_m_contracargo_mastercard:
            steps:
                - invoque_prc_load_table_m_contracargo_mastercard:
                    call: SyncBigQueryJob
                    args:
                        query_pt1: ${"CALL `" + procedures.m_contracargo_mastercard.name + "`('"
